...
var PIPE=[
 {t:"Decode", h:"DecodeProgram — RDNA 2 words → typed instructions",
  b:"<p><code>TranslateProgram()</code> starts here. Walk the code blob and decode each instruction family, recording opcode, operands, literal constants, modifier bits, raw words and program counter. Mesh shaders use <code>DecodeFusedProgram()</code> instead, joining the front and back stage code into one stream with an ordinary CFG edge between them.</p><p>The rest of the compiler sees typed instructions rather than packed machine words.</p>",
  w:"Everything downstream works on a typed instruction list rather than raw bits. An unrecognised instruction is a locatable failure, not silent nonsense.",
 ...
 {t:"Structurize", h:"Structurize — or dispatcher fallback",
  b:"<p>SPIR-V requires every conditional branch to declare a merge block and every loop to declare both a merge and a continue target. This pass computes them, using nearest-common-post-dominator for selections and loop-exit analysis for loops.</p><p>On irreducibility or a failed structurize the graph is restored unstructured, <code>dispatcher_fallback</code> is set with a <code>FailureKind</code>, and emission later uses a program-counter dispatcher.</p>",
  w:"This is where GPU control flow — which is mask manipulation, not structure — becomes something a structured IR can express. Failure is a documented fallback, not an error.",
  s:"…/frontend/cfg/ShaderCFG.cpp"},
 {t:"Translate", h:"DetectEmbeddedVertexFetch, then Frontend::TranslateProgram",
  b:"<p>On a vertex shader with an embedded fetch prologue, <code>DetectEmbeddedVertexFetch()</code> builds a plan first. Category dispatchers then translate scalar, vector, memory, LDS and export instructions into the typed IR.</p>",
 ...
 {t:"SSA + simplify", h:"RewriteToSsa through EliminateReadLane",
