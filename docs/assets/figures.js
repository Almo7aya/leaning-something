...
/* ---------------- shader pipeline ---------------- */
var PIPE=[
 {t:"Decode", h:"DecodeProgram — RDNA 2 words → instruction list",
  b:"<p>Walk the code blob and decode each instruction from its encoding family (SOP1, SOP2, SOPP, VOP1/2/3, VOPC, SMEM, MUBUF, MTBUF, MIMG, FLAT, DS, EXP), recording opcode, operands, literal constants, modifier bits and program counter.</p><p>Instructions are one to five words long, and the decoder must know each family's operand layout to find the boundary.</p>",
  ...
  s:"…/decompiler/ShaderDecoder.cpp"},
 ...
 {t:"Structurize", h:"Structurize — find merge and continue blocks",
  ...
  s:"…/cfg/ShaderCFG.cpp"},
 {t:"Lower to IR", h:"LowerProgram — machine ops → IR opcodes",
  b:"<p>Translate each decoded instruction into one or more IR instructions from a set of several hundred opcodes. Register operands become IR values; the wave size and lane-mask mode are attached to the program here, because they change what several instructions mean.</p>",
  w:"The IR stays deliberately close to the machine. This is a translator, not an optimising compiler, and closeness keeps the mapping auditable when a shader renders incorrectly.",
  s:"…/ir/ShaderIR.cpp"},
 {t:"Provenance", h:"BuildScalarProvenance — where did this scalar come from?",
  b:"<p>A dataflow analysis over scalar registers. Every scalar value is given an origin: a user-data slot, a constant, an arithmetic combination of known values, a <code>ReadConst</code> memory load, a phi node — or unknown.</p>",
  w:"Descriptors are loaded from addresses the shader computes. Without knowing how those addresses were built there is no way to know which resource to bind, so this analysis is what makes descriptor recovery possible at all.",
  s:"…/ir/ScalarProvenance.cpp"},
 {t:"SRT plan", h:"BuildSrtPlan — a recipe for reading descriptor tables",
  b:"<p>Collect every reachable <code>ReadConst</code> — the shader reading its own resource table — and turn it into a plan. Constant offsets are assigned compact flat-buffer slots; genuinely dynamic offsets stay explicit and are never given a fake slot.</p>",
  ...
  s:"…/ir/SrtWalker.cpp"},
 {t:"Track resources", h:"PatchSrtReads + TrackResources",
  b:"<p>Rewrite descriptor reads to reference the plan, then classify every resource: buffers (read, written, atomic, formatted, scalar), images (kind, dimension, cube, depth-compare, storage swizzle), samplers, and sampled image/sampler pairs.</p>",
  ...
  s:"…/ir/ResourceTracking.cpp"},
 {t:"Materialize", h:"MaterializeResources + SpecializeResources",
  b:"<p>Run the plan against actual user data and actual guest memory to obtain concrete descriptors, then specialise the IR against what was found — a buffer whose stride is now known, or an image whose format is now known, can be emitted more directly.</p>",
  ...
  s:"…/ir/ResourceMaterialization.cpp"},
 {t:"Bindings", h:"CollectShaderInfo + AllocateBindings",
  b:"<p>Gather stage-specific facts — which vertex attributes are read, which pixel interpolators, workgroup size for compute — then assign descriptor set numbers, binding indices and push-constant offsets (up to 32 dwords).</p>",
  ...
  s:"…/ir/BindingLayout.cpp"},
 {t:"Optimise", h:"EliminateReadLane",
  b:"<p><code>v_readlane</code> and <code>v_readfirstlane</code> read one lane of a vector register into a scalar. These map poorly onto SPIR-V subgroup operations. Where the value is provably uniform, the pass rewrites the read into a direct use and drops the cross-lane operation.</p>",
  w:"Subgroup operations are the least portable part of SPIR-V. Every one removed is one fewer host-driver difference to debug.",
  s:"…/ir/ReadLaneElimination.cpp"},
 {t:"Emit SPIR-V", h:"Spirv::EmitProgram",
  b:"<p>The largest stage — about 516 KB across a dozen files. Declares types, capabilities, the entry point, interface variables and descriptors, then emits every IR instruction as SPIR-V, including all the lowering the hardware gets for free: bounds-checked buffer access, image format conversion, packed arithmetic.</p><p>Optionally validated with <code>spirv-val</code> and optimised with <code>spirv-opt</code> before reaching the driver.</p>",
