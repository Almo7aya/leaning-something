(function () {
  ...
  Cache.prototype.commit = function (key) {
    this.misses++;
    var pipe = { key: key, name: FX[key] || ("perm-" + key), hash: (0x51A00000 + key * 0x20F) >>> 0 };
  ...
  Emu.prototype.advanceBoot = function (dt) {
    ...
    if (this.bootAnim > 6) this.finishBoot();
  };
  Emu.prototype.finishBoot = function () {
    this.booted = true; this.bootAnim = null; this.pipeOn = 6;
  ...
  Emu.prototype.stepFrame = function () {
    ...
    if ((this.knobMode | 0) >= GAME0) this.gameStep();
  ...
  Emu.prototype.sysSubmit = function () {
    ...
    this.pipeOn = 11;
    if (!this.shaders.has(mode)) { this.startCompile(mode); return; }
    this.shaders.hit(mode);
    this.finishSubmit(mode);
  };
  Emu.prototype.startCompile = function (mode) {
    this.compileAnim = { key: mode, stage: 0, t: 0 };
    this.shaders.stage = 0;
    this.log("gpu", "specialisation miss " + (FX[mode] || ("perm-" + mode)) + " → compile", "gpu");
  };
  Emu.prototype.advanceCompile = function (dt) {
    ...
    if (ca.t >= 105) {
      ...
      if (ca.stage <= 4) this.log("gpu", SH[ca.stage] + " " + (FX[ca.key] || ca.key), "gpu");
    }
    if (ca.stage > 4) {
      var key = ca.key;
      ...
      this.log("gpu", "CompileProgram done 0x" + hex(this.shaders.pipes[key].hash), "gpu");
    }
    this.draw();
  };
  Emu.prototype.finishSubmit = function (mode) {
  ...
  Emu.prototype.draw = function () {
    ...
    var showMode = this.compileAnim ? this.compileAnim.key : mode;
  ...
  Emu.prototype.compileOverlay = function (c) {
    ...
    c.fillText("◷ COMPILING  " + (FX[ca.key] || ca.key), 320, 150);
  ...
  Emu.prototype.pipeInspect = function (i) {
    ...
    switch (i) {
      ...
      case 10: return tbl(this.pm4.length ? this.pm4.map(function (p) { return [p.name, "n=" + p.n + (p.pl.length ? " [" + p.pl.join(",") + "]" : "")]; }) : [["(idle)", "no packet yet"]]);
  ...
  Emu.prototype.shInspect = function (i) {
    var mode = this.m.peek(U.mode) | 0, self = this;
    var name = FX[mode] || ("perm-" + mode);
    if (i === 0) return tbl([["source", name], ["typed ops", "load_uniform, mul, sin, mix, store"], ["kind", mode >= GAME0 ? "game blit" : "fullscreen fs"]]);
    if (i === 1) return tbl([["blocks", "3"], ["branches", mode === 1 ? "1 (tunnel guard)" : "0"], ["reducible", "yes"]]);
    if (i === 2) return tbl([["IR", "TranslateProgram → typed value IR"], ["ssa values", 18 + (mode % 5) * 4], ["uniforms read", "tick, mode, warp, bass"]]);
    if (i === 3) { var m = this.m; return tbl([["tick", hex(m.peek(U.tick))], ["mode", hex(m.peek(U.mode))], ["warp", hex(m.peek(U.warp))], ["bass", hex(m.peek(U.bass))]]); }
    if (i === 4) { var p = this.shaders.pipes[mode]; return tbl([["specialised", p ? "yes" : "compiling…"], ["hash", p ? "0x" + hex(p.hash) : "—"], ["cache", this.shaders.hits + " hit / " + this.shaders.misses + " miss"]]); }
    return "";
  };
